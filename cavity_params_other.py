...
vel_ph = 0.02           # uniform velocity in on left side 0.2 m/s (characteristic vel macroscale)
height = 0.2    # height of channel 2cm = 0.02m (characteristic length macroscale)
width = 0.2    # length of channel 50cm = 0.5m
...
M = 101                  # lattices in y direction
N = int(aspect * M)      # lattices in x direction, we want deltax = deltay

reynolds_lattice = reynolds
velocity_lattice = vel_ph * 10    # choose so we can keep the other values same (sane?)
#reynolds_lattice = velocity_lattice * M / viscosity     # keep this same as macroscale
viscosity_lattice = velocity_lattice * M / reynolds_lattice   # use deltax = 1 so L = N (discretized width)
...
print 'velocity_lattice:'+str(velocity_lattice)
